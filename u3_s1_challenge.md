...
#Answers: 
* Code reviews have many important elements that matter when considering what the code is doing in the business from a holistic standpoint. The code must be legible, readable, scalable, commented, and efficient to name a few qualities, and a good review will ensure the code has all these. Remember, the objective of good code
  is to serve its functional purpose in the present, while being written in a way that is scalable, permutable, and explainable in the future. 
  Therefore I would argue that the most important part of a code review is the holistic evaluation of the code from the point of view of future stakeholders.
  
* Containers offer a relatively pain-free solution to the problem of heterogeneous operating environments. Containers can be set up 
quickly, to emulate any sort of operating environment, in order to test our code to ensure function in these settings. Containers
   can be relatively pain-free as software like Docker manages creation, deployment, and the running of these containers for you.
